...
signed main(int argc, char* argv[]) {
  ...
  priority_queue<int64_t, vector<int64_t>, greater<int64_t>> right;
  int64_t center = 0;
  int64_t sum_b  = 0;
  int64_t sum_xa = 0;
  rep(i, Q) {
    ...
    if (com == 1) {
      ...
      sum_xa += abs(a - center);
      while (left.size() > size_t(m)) {
        int64_t new_center = left.top();
        int64_t d          = abs(center - new_center);
        sum_xa -= int64_t(left.size()) * d;
        left.pop();
        right.push(center);
        center = new_center;
        sum_xa += int64_t(right.size()) * d;
      }
      while (left.size() < size_t(m)) {
        int64_t new_center = right.top();
        int64_t d          = abs(new_center - center);
        sum_xa -= int64_t(right.size()) * d;
        right.pop();
        left.push(center);
        center = new_center;
