...
signed main(int argc, char* argv[]) {
  ...
  vector<int64_t> S;
  S.reserve(N);
  rep(i, N) S.emplace_back(S_in[i] - '0');
  ...
  cnt.emplace_back(tmp);
  vector<int64_t> cum(cnt.size() + 1, 0);
  rep(i, cnt.size()) { cum[i + 1] = cum[i] + cnt[i]; }

  int64_t ans = 0;
  for (int64_t i = (S[0] == 0 ? 0 : 1); i < int64_t(cnt.size()); i += 2) {
    int64_t l = max<int64_t>(0, i - 1);
    int64_t r = min<int64_t>(cnt.size(), i + 2 * (K - 1) + 1 + 1);
    ChMax(ans, cum[r] - cum[l]);
  }
  cout << ans << endl;
  return EXIT_SUCCESS;
}
