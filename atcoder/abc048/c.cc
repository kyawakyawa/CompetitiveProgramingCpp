...
int64_t Check(int64_t N, int64_t x, const vector<int64_t> a_) {
  vector<int64_t> a = a_;
  int64_t ret       = 0;

  rep(i, 1, N) {
    int64_t tmp = max(int64_t(0), a[i - 1] - x);
    ret += tmp;
    a[i - 1] -= tmp;

    tmp = a[i - 1] + a[i];
    tmp = max(int64_t(0), tmp - x);
    ret += tmp;
    a[i] -= tmp;
  }
...
signed main(int argc, char* argv[]) {
  ...
  vector<int64_t> a(N);
  rep(i, N) In(a[i]);
  int64_t ans = Check(N, x, a);
  reverse(a.begin(), a.end());
  ChMax(ans, Check(N, x, a));
  cout << ans << endl;
  return EXIT_SUCCESS;
}
