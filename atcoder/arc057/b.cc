...
signed main(int argc, char* argv[]) {
  ...
  auto dp = MakeNdVector(N + 1, N + 1, kInf);
  ...
  rep(i, N) {
    rep(j, N + 1) {
      ...
      if (sum_a == 0) {
        ChMin(dp[i + 1][j + 1], int64_t(1));
      } else {
        ChMin(dp[i + 1][j + 1], (dp[i][j] * (sum_a + a[i]) + sum_a) / sum_a);
      }
    }
    sum_a += a[i];
  }

  cout << ALL(max_element, dp[N],
              [&K](int64_t x, int64_t y) {
                if (x > K) x = 0;
                if (y > K) y = 0;
                return x < y;
              }) -
