...
signed main(int argc, char* argv[]) {
  ...
  cin >> s_in;
  vector<int64_t> s(N);
  rep(i, N) s[i] = s_in[i] - '0';

  vector<vector<int64_t>> next(N, vector<int64_t>(10, N));

  {
    vector<int64_t> next_start(10, 0);

    rep(i, 0, N) {
      rep(j, next_start[s[i]], i + 1) { next[j][s[i]] = i; }
      next_start[s[i]] = i + 1;
    }
  }
  ...
  rep(i, 0, N) {
    rep(j, 3) {
      rep(k, 10) {
        int64_t id = next[i][k];
        if (id < N) dp[id + 1][j + 1] += dp[i][j];
      }
    }
  }

  int64_t ans = 0;
  rep(i, 1, N + 1) { ans += dp[i][3]; }
