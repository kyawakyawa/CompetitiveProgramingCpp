...
int64_t Hoge(int64_t n, int64_t p) {
  int64_t ret = 0;
  rep(i, p) {
    if (n & (int64_t(1))) ret += (int64_t(1) << i);
    n /= 2;
  }
  return ret;
}
int64_t Bit(int64_t n, int64_t p) { return (n >> p) & int64_t(1); }
...
signed main(int argc, char* argv[]) {
  ...
  int64_t ans = 0;
  rep(i, n) {
    int64_t x   = A / (int64_t(1) << i);
    int64_t y   = B / (int64_t(1) << i);
    int64_t cnt = 0;
    if (x % 2 == 0) {
      cnt += ((y - x + 1) / 2) * (int64_t(1) << i);
    } else {
      cnt += ((y - x + 2) / 2) * (int64_t(1) << i);
    }
    if (x % 2 == 1) {
      cnt -= A % (int64_t(1) << i);
    }
    if (y % 2 == 1) {
      cnt -= (int64_t(1) << i) - B % (int64_t(1) << i) - 1;
    }
