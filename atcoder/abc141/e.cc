...
signed main(int argc, char* argv[]) {
  ...
  RollingHash<1000000007> rhs(S);

  auto Comp = [&](const int64_t len) {
    for (int64_t i = 0; i + len * 2 <= N; ++i) {
      for (int64_t j = i + len; j + len <= N; ++j) {
        if (rhs.Get(i, i + len) == rhs.Get(j, j + len)) {
          return false;
        }
      }
    }
  ...
  cout << LowerBound<int64_t>(0, N + 1, Comp) - 1 << endl;
  return EXIT_SUCCESS;
}
