...
signed main(int argc, char* argv[]) {
  ...
  if (A[0] + A[1] + A[2] == 1) {
    vector<string> select(N);
    rep(i, N) {
      if (A[s[i][0] - 'A'] == 0 && A[s[i][1] - 'A'] == 0) {
        cout << "No" << endl;
        return EXIT_SUCCESS;
      }
      swap(A[s[i][0] - 'A'], A[s[i][1] - 'A']);
      if (A[s[i][0] - 'A'] == 1) {
        select[i] = s[i][0];
      } else {
        select[i] = s[i][1];
    ...
    cout << "Yes" << endl;
    rep(i, N) cout << select[i] << endl;
    return 0;
  }
  cout << Yes(!(A[s[0][0] - 'A'] == 0 && A[s[0][1] - 'A'] == 0)) << endl;

  if (!(A[s[0][0] - 'A'] == 0 && A[s[0][1] - 'A'] == 0)) {
    rep(i, N - 1) {
      if (A[s[i][0] - 'A'] == 0) {
        A[s[i][0] - 'A'] += 1;
        A[s[i][1] - 'A'] -= 1;
