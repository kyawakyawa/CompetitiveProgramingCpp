...
signed main(int argc, char* argv[]) {
  ...
  int64_t now = a;
  used[a]     = true;
  cycle.emplace_back(a);
  while (true) {
    if (used[b[now]]) {
      st = b[now];
      break;
    }
    cycle.emplace_back(b[now]);
    used[b[now]] = true;
    now          = b[now];
  }
  int64_t sti = 0;
  rep(i, cycle.size()) if (cycle[i] == st) {
    sti = i;
    break;
  }

  int64_t m = int64_t(cycle.size()) - sti;

  if (k.size() < 7) {
    int64_t k_ = atol(k.c_str());
    if (k_ < sti) {
      cout << cycle[k_] + 1 << endl;
    } else {
      cout << cycle[sti + (k_ - sti) % m] + 1 << endl;
    }
  } else {
    int64_t p  = 1;
    int64_t k_ = 0;
