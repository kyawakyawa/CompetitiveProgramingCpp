...
signed main(int argc, char* argv[]) {
  ...
  rep(S, 1 << (H - 1)) {
    ...
    vector<int64_t> d_ids(H, 0);
    rep(i, (H - 1)) {
      if (bs[i]) {
        d_ids[i + 1] = 1;
      }
    }
    rep(i, (H - 1)) { d_ids[i + 1] += d_ids[i]; }

    vector<int64_t> cnts(d_ids.back() + 1, 0);

    int64_t cnt = 0;
    rep(j, W) {
      bool flag = false;
      vector<int64_t> _cnts(d_ids.back() + 1, 0);
      rep(i, H) {
        int64_t d_id = d_ids[i];
        if (s[i][j] == '1') {
          _cnts[d_id]++;
        }
        if (cnts[d_id] + _cnts[d_id] > K) {
          flag = true;
        }
      }
      if (flag) {
      ...
      if (flag) {
        cnt = numeric_limits<int64_t>::max() - 100;
        break;
      }
    }
    ChMin(ans, cnt + d_ids.back());
  }
  cout << ans << endl;
  return EXIT_SUCCESS;
}
