...
signed main(int argc, char* argv[]) {
  ...
  Graph<int64_t> graph(N * (N - 1) / 2);
  rep(i, N) {
    rep(j, N - 2) {
      int64_t from = ids[i][A[i][j]];
      int64_t to   = ids[i][A[i][j + 1]];
      graph[from].emplace_back(to);
    }
  }

  vector<int64_t> sorted, group;
  if (TopologicalSort(graph, &sorted, &group)) {
    cout << *ALL(max_element, group) + 1 << endl;
  } else {
    cout << -1 << endl;
  }
