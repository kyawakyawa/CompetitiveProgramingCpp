...
signed main(int argc, char* argv[]) {
  ...
  rep(i, log2) {
    uvw.emplace_back(Pp(P(i + 1, i + 2), 0));
    uvw.emplace_back(Pp(P(i + 1, i + 2), int64_t(1) << i));
  }

  {
    int64_t i    = 1;
    int64_t next = (int64_t(1) << log2);
    while (b > 0) {
      if (b & 1) {
        uvw.emplace_back(Pp(P(i, log2 + 1), next));
        next += (int64_t(1) << (i - 1));
      }
      b /= 2;
      i++;
