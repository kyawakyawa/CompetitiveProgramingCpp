...
signed main(int argc, char* argv[]) {
  ...
  V.resize(N + 1);
  W.resize(N + 1);
  rep(i, 1, N + 1) In(V[i], W[i]);
  int64_t Q;
  In(Q);
  using P = pair<int64_t, int64_t>;
  vector<vector<P>> ql(N + 1);
  rep(i, Q) {
    int64_t v, L;
    In(v, L);
    ql[v].emplace_back(i, L);
  }

  vector<int64_t> ans(Q);

  int64_t T    = min(N, int64_t(511));
  const int MX = 100000;
  auto dp      = MakeNdVector(512, MX + 1, int64_t(0));
  for (int64_t i = 1; i <= T; ++i) {
    for (int64_t j = 0; j <= MX; ++j) {
      int64_t v = dp[i >> int64_t(1)][j];
      ChMax(dp[i][j], v);
      if (j + W[i] <= MX) ChMax(dp[i][j + W[i]], v + V[i]);
    }
    for (auto& [q, l] : ql[i]) ans[q] = dp[i][l];
  }

  vector<int64_t> vm(1 << 18), wm(1 << 18);
  ...
  for (int64_t i = T + 1; i <= N; ++i) {
    ...
    int64_t sz  = ps.size();
    int64_t msb = 1, idx = 0;
    for (int64_t b = 0; b < (1 << sz); ++b) {
      if (b == 2 * msb) {  // 桁上がる 最上位のもの + それ以下のもので計算
        msb = b;
        idx++;
      }
      int64_t v = 0, w = 0;
