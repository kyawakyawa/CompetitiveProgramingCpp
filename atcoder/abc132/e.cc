...
signed main(int argc, char* argv[]) {
  ...
  const int64_t kInf = numeric_limits<int64_t>::max() / 4;
  ...
  auto use = MakeNdVector(N, 4, 0);

  while (!que.empty()) {
    int64_t now = que.front();
    que.pop();

    const function<set<int64_t>(int64_t, int64_t)> Dfs = [&](int64_t v,
                                                             int64_t dpt) {
      set<int64_t> ret;
      if (use[v][dpt]) {
        return ret;
      ...
      for (const auto u : g[v]) {
        ret.merge(Dfs(u, dpt + 1));
      }

      return ret;
    };

    set<int64_t> nexts = Dfs(now, 0);
