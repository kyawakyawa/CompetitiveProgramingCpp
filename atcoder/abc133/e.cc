...
Mi Cnt(int64_t par, int64_t dpt, const Combination<Mi> &combi,
       vector<int64_t> *used) {
  vector<Mi> tmp;
  for (auto v : tr[par]) {
    if (!used->at(v)) {
      used->at(v) = true;
      tmp.emplace_back(Cnt(v, dpt + 1, combi, used));
    }
  }
  int64_t num_ch = int64_t(tmp.size());
  ...
  Mi p = 0;
  if (dpt == 0) {
    p = combi.P(K - 1, num_ch);
  } else {
    p = combi.P(K - 2, num_ch);
  }

  return p *
         ALL(accumulate, tmp, Mi(1), [](Mi init, Mi v) { return init * v; });
}
...
signed main(int argc, char *argv[]) {
  ...
  Combination<Mi> combi(K);
  vector<int64_t> used(N, false);
  used[0] = true;
  cout << Mi(K) * Cnt(0, 0, combi, &used) << endl;
  return EXIT_SUCCESS;
}
