...
signed main(int argc, char* argv[]) {
  ...
  ALL(sort, p, greater<int64_t>());
  ALL(sort, q, greater<int64_t>());

  vector<int64_t> eaten;
  eaten.reserve(X + Y + C);

  rep(i, X) eaten.emplace_back(p[i]);
  rep(i, Y) eaten.emplace_back(q[i]);
  rep(i, C) eaten.emplace_back(r[i]);

  ALL(sort, eaten, greater<int64_t>());

  cout << accumulate(eaten.begin(), eaten.begin() + X + Y, int64_t(0)) << endl;
