...
signed main(int argc, char* argv[]) {
  ...
  int64_t m;
  In(m);
  vector<int64_t> xm(m), ym(m);
  rep(i, m) In(xm[i], ym[i]);
  int64_t n;
  In(n);
  vector<int64_t> xn(n), yn(n);
  rep(i, n) In(xn[i], yn[i]);

  rep(i, n) {
    int64_t dx = xn[i] - xm[0];
    int64_t dy = yn[i] - ym[0];
    bool flag  = true;
    rep(j, m) {
      int64_t x = xm[j] + dx;
      int64_t y = ym[j] + dy;
