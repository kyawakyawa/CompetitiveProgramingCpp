...
int64_t Cnt(int64_t n, int64_t x, const vector<int64_t>& hs,
            const vector<int64_t>& num_p) {
  int64_t h = hs[n];
  ...
  if (x < (h + 1) / 2) {
    return Cnt(n - 1, x - 1, hs, num_p);
  }

  return Cnt(n - 1, x - (h + 1) / 2, hs, num_p) + num_p[n - 1] + 1;
}
...
signed main(int argc, char* argv[]) {
  ...
  auto num_p = MakeNdVector(N + 1, int64_t(1));
  auto hs    = MakeNdVector(N + 1, int64_t(1));
