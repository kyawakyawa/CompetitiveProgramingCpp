...
signed main(int argc, char* argv[]) {
  ...
  int64_t sum_l = ALL(accumulate, l, 0);
  int64_t sum_r = ALL(accumulate, r, 0);

  bool flag0 = (sum_l == sum_r);
  ...
  rep(i, N) {
    if (r[i] >= l[i]) {
      inc.emplace_back(l[i], -r[i]);
    } else {
      dec.emplace_back(r[i], -l[i]);
    }
  }
  ...
  cout << Yes(Check(inc) && Check(dec)) << endl;
