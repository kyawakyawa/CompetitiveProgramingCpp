...
signed main(int argc, char* argv[]) {
  ...
  array<int64_t, 3> sigma = {int64_t(1), int64_t(2), int64_t(3)};

  int64_t ans = 0;
  do {
    vector<int64_t> s(n);
    rep(i, n) { s[i] = sigma[s_ori[i]]; }

    rep(i, n) {
      if (s[i] != 2) continue;
      vector<int64_t> cnt1(i);
      vector<int64_t> cnt3(n - i - 1);
      int64_t sum1 = 0;
      int64_t sum3 = 0;
      rep(j, i) {
        if (s[j] == 1) {
      ...
      int64_t tmp = sum1 * sum3;
      rep(j, min(cnt1.size(), cnt3.size())) { tmp -= cnt1[j] * cnt3[j]; }
      ans += tmp;
    }
