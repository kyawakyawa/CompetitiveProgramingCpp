...
signed main(int argc, char* argv[]) {
  ...
  vector<BinaryIndexedTree<int64_t>> bits(26, N + 1);

  rep(i, N) { bits[s[i] - 'a'].Add(i + 1, 1); }
  ...
  rep(i, Q) {
    ...
    if (t == 1) {
      ...
      iq--;
      char now_c = s[iq];
      bits[now_c - 'a'].Add(iq + 1, -1);
      bits[cq - 'a'].Add(iq + 1, 1);
      s[iq] = cq;
    } else {
      int64_t lq, rq;
      In(lq, rq);
      int64_t out = 0;
      rep(c, 'z' - 'a' + 1) {
        const int64_t tmp = bits[c].Sum(rq) - bits[c].Sum(lq - 1);
        if (tmp > 0) {
          out++;
        }
      }
