...
signed main(int argc, char* argv[]) {
  ...
  // vector<tuple<int64_t, int64_t, int64_t>> hxy(N);
  rep(i, N) {
    In(x[i], y[i], h[i]);
    // hxy[i] = make_tuple(h[i], x[i], y[i]);
  }
  // ALL(sort, hxy, greater<tuple<int64_t, int64_t, int64_t>>());
  // rep(i, N) {
  //   x[i] = get<1>(hxy[i]);
  //   y[i] = get<2>(hxy[i]);
  //   h[i] = get<0>(hxy[i]);
  ...
  rep(Cy, 101) {
    rep(Cx, 101) {
      ...
      bool flag  = true;
      rep(i, N) {
        int64_t dy = abs(y[i] - Cy);
        int64_t dx = abs(x[i] - Cx);

        if (h[i] > 0) {
          if (H < kInf && h[i] + dy + dx != H) {
            flag = false;
            break;
          }
          H = h[i] + dy + dx;
