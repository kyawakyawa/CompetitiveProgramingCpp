...
Mi Combi(int64_t n, int64_t k) {
  Mi bunbo  = 1;
  Mi bunshi = 1;
  rep(i, k) { bunshi = bunshi * Mi(n - i); }
  rep(i, k) { bunbo = bunbo * Mi(i + 1); }

  return bunshi * ModPow<int64_t>(bunbo.x, mod - 2, mod);
}
signed main(int argc, char *argv[]) {
  ...
  cout << Mi(ModPow<int64_t>(2, n, mod)) - Combi(n, a) - Combi(n, b) - Mi(1)
       << endl;
