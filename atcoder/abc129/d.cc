...
int64_t Cnt(int64_t cy, int64_t cx, int64_t d) {
  if (S[cy][cx] == '#') return 0;
  ...
  int64_t x = cx + dx[d];
  int64_t y = cy + dy[d];
  if (x < 0 || W <= x || y < 0 || H <= y || S[y][x] == '#') {
    return memo[cy][cx][d] = 0;
  }

  return memo[cy][cx][d] = 1 + Cnt(y, x, d);
}
...
signed main(int argc, char* argv[]) {
  ...
  rep(i, H) { In(S[i]); }
  memo.resize(H);
  rep(i, H) { memo[i].resize(W, vector<int64_t>(4, -1)); }
