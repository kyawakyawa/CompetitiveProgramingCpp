...
signed main(int argc, char* argv[]) {
  ...
  vector<int64_t> AB;
  rep(i, X) {
    rep(j, Y) { AB.emplace_back(A[i] + B[j]); }
  }

  ALL(sort, AB, greater<int64_t>());
  ALL(sort, C, greater<int64_t>());
  ...
  priority_queue<pair<int64_t, P>> que;

  rep(i, S.size()) { que.emplace(S[i] + T[0], P(i, 0)); }

  rep(k, K) {
    auto s = que.top();
    que.pop();
    cout << s.first << endl;
    if (size_t(s.second.second + 1) < T.size()) {
      int64_t sum = S[s.second.first] + T[s.second.second + 1];
      que.emplace(sum, P(s.second.first, s.second.second + 1));
    }
  }
