...
signed main(int argc, char* argv[]) {
  ...
  rep(i, M) In(P[i], Y[i]);
  using Tpl = tuple<int64_t, int64_t, int64_t>;
  vector<Tpl> ypi;
  rep(i, M) { ypi.emplace_back(Y[i], P[i], i); }
  sort(ypi.begin(), ypi.end());
  map<int64_t, int64_t> cnts;

  vector<string> ans(M);
  rep(i, M) {
    int64_t p = get<1>(ypi[i]);
    cnts[p]++;
    int64_t x = cnts[p];
    stringstream ss;
    ss << std::setfill('0') << std::right << std::setw(6) << p;
    ss << std::setfill('0') << std::right << std::setw(6) << x;
    ans[get<2>(ypi[i])] = ss.str();
  }
