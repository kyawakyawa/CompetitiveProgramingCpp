...
int64_t ModPow(int64_t a, int64_t x, int64_t mod) {
  int64_t ret = 1;
  while (x > 0) {
    if (x & 1) {
      ret *= a;
...
signed main(int argc, char* argv[]) {
  ...
  rep(i, 1, n + 1) {
    int64_t tmp = C[i - 1];
    tmp *= ModPow(2, i - 1, mod);
    tmp %= mod;
    int64_t m    = n - i + 1;
    int64_t hoge = (m - 1) * ModPow(2, m - 2, mod) % mod;
    hoge += ModPow(2, m - 1, mod);
    hoge %= mod;
    ans += tmp * hoge % mod;
    ans %= mod;
  }
  Out(ans * ModPow(2, n, mod) % mod);
