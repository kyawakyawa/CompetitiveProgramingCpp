...
signed main(int argc, char* argv[]) {
  ...
  rep(i, n) { In(xy[i].first, xy[i].second); }
  ...
  rep(i, n) {
    rep(j, i + 1, n) {
      ...
      int64_t a = A.second - B.second;
      int64_t b = -(A.first - B.first);

      if (s.count(P(A.first + a, A.second + b)) > 0 &&
          s.count(P(B.first + a, B.second + b)) > 0) {
        ChMax(ans, a * a + b * b);
      }
      if (s.count(P(A.first - a, A.second - b)) > 0 &&
          s.count(P(B.first - a, B.second - b)) > 0) {
