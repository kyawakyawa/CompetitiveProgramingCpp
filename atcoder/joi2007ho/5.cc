...
int64_t Lcm(const int64_t x, const int64_t y) { return x / Gcd(x, y) * y; }

int64_t Dfs(int64_t v) {
  int64_t lw = 1;
  if (r[v] >= 0) {
    lw *= Dfs(r[v]);
  }
  ...
  int64_t lcm = Lcm(p[v] * lw, q[v] * rw);

  return lcm / p[v] + lcm / q[v];
}
