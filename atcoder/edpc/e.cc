...
signed main(int argc, char* argv[]) {
  ...
  int64_t N, W;
  In(N, W);
  auto w = MakeNdVector(N, int64_t(0));
  auto v = MakeNdVector(N, int64_t(0));

  rep(i, N) { In(w[i], v[i]); }

  const int64_t sum_v = ALL(accumulate, v, 0);

  auto dp = MakeNdVector(N + 1, sum_v + 1, numeric_limits<int64_t>::max() / 3);
