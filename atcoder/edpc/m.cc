...
signed main(int argc, char *argv[]) {
  ...
  auto dp  = MakeNdVector(N + 1, K + 1, Mi(0));
  auto cum = MakeNdVector(K + 2, Mi(0));
  dp[0][0] = 1;
  rep(i, K + 1) { cum[i + 1] = cum[i] + dp[0][i]; }

  rep(i, N) {
    rep(j, K + 1) {
      int64_t l    = max<int64_t>(0, j - a[i]);
      int64_t r    = min<int64_t>(K + 1, j + 1);
      dp[i + 1][j] = cum[r] - cum[l];
    }
    ALL(fill, cum, Mi(0));
    rep(j, K + 1) { cum[j + 1] = cum[j] + dp[i + 1][j]; }
  }
  cout << dp[N][K] << endl;
