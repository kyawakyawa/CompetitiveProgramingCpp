...
signed main(int argc, char* argv[]) {
  ...
  int64_t L = ALL(accumulate, a, int64_t(0));

  int64_t X = 0, Y = 0;
  rep(i, N) if (a[i] == 3) X++;
  rep(i, N) if (a[i] == 2) Y++;

  auto dp = MakeNdVector(L + 1, N + 1, N + 1, 0.0);

  rep(i, 1, L + 1) {
    rep(j, N + 1) {
      rep(k, N - j + 1) {
        int64_t l = i - 3 * j - 2 * k;
        if (l < 0 || N - (j + k + l) < 0) continue;
        double d   = double(N - (j + k + l)) / double(N);
        double c   = double(l) / double(N);
        double b   = double(k) / double(N);
        double aa  = double(j) / double(N);
        double tmp = d;
        if (j - 1 >= 0 && k + 1 <= N) {
          tmp += aa * (dp[i - 1][j - 1][k + 1] + 1.);
        }
        if (k - 1 >= 0) {
          tmp += b * (dp[i - 1][j][k - 1] + 1.);
        }
        tmp += c * (dp[i - 1][j][k] + 1.);

        dp[i][j][k] = tmp / (1.0 - double(d));
      }
    }
  }
  printf("%.10f\n", dp[L][X][Y]);
