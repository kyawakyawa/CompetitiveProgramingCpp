...
signed main(int argc, char* argv[]) {
  ...
  const auto comp    = [&](int64_t e) {
    using P  = pair<int64_t, int64_t>;
    using PP = pair<int64_t, pair<int64_t, int64_t>>;
    priority_queue<PP, vector<PP>, greater<PP>> que;
    auto d = MakeNdVector(H, W, kInf);

    d[sy][sx] = 0;
    que.emplace(0, P(sy, sx));

    while (!que.empty()) {
      PP p = que.top();
      que.pop();

      int64_t y = p.second.first;
      int64_t x = p.second.second;

      rep(i, 4) {
        int64_t y_ = y + dy[i];
        int64_t x_ = x + dx[i];
        if (y_ < 0 || H <= y_ || x_ < 0 || W <= x_) continue;
        int64_t d_ = d[y][x];
        if (s[y_][x_] == '#') {
          d_ += e;
        } else {
  ...
  cout << LowerBound<int64_t>(int64_t(0), T + 1, comp) - 1 << endl;
  return EXIT_SUCCESS;
}
