...
int64_t ToOut(const vector<int64_t>& ar) {
  int64_t out = 0;
  int64_t ten = 1;
  for (const auto v : ar) {
    out += v * ten;
...
signed main(int argc, char* argv[]) {
  ...
  rep(k, 1, K + 1) {
    ...
    rep(i, 0, d - 1) {
      if (ans[i] <= ans[i + 1] && ans[i] < 9) {
        ans[i] += 1;
        rrep(j, 0, i) { ans[j] = max(int64_t(0), ans[j + 1] - 1); }
        flag = true;
        break;
      }
    }
    ...
    rrep(j, 0, d - 1) { ans[j] = max(int64_t(0), ans[j + 1] - 1); }
  }
  cout << ToOut(ans) << endl;
  return EXIT_SUCCESS;
}
