...
signed main(int argc, char* argv[]) {
  ...
  rep(i, N) In(A[i]);
  int64_t d = NumD(K);
  rep(i, N) { ChMax(d, NumD(A[i])); }

  vector<int64_t> cnt0(d, 0), cnt1(d, 0);
  rep(i, N) {
    rep(j, d) {
      int64_t a = ((A[i] >> j) & 1);
      if (a == 1) {
  ...
  auto dp = MakeNdVector(d + 1, 2, int64_t(-kInf));
  ...
  rep(i, d) {
    if (dp[i][false] >= 0) {
      if (k[i] == 0) {
        ChMax(dp[i + 1][false], dp[i][false] * 2 + cnt1[i]);
      } else {
        ChMax(dp[i + 1][false], dp[i][false] * 2 + cnt0[i]);
        ChMax(dp[i + 1][true], dp[i][false] * 2 + cnt1[i]);
      }
    }
    if (dp[i][true] >= 0) {
      ChMax(dp[i + 1][true], dp[i][true] * 2 + max(cnt0[i], cnt1[i]));
    }
  }

  cout << max(dp[d][false], dp[d][true]) << endl;
