...
void Dfs2(int64_t v) {
  ...
  for (auto u : g[v]) {
    if (!visit[u]) {
      tmps.emplace_back(dp[u]);
      Dfs2(u);
    }
  }
  ans[v] = 0;
  vector<int64_t> cum(tmps.size() + 1, 0);
  rep(i, tmps.size()) { cum[i + 1] = cum[i] + tmps[i]; }

  rep(i, int64_t(tmps.size()) - 1) {
    ans[v] += tmps[i] * (cum[tmps.size()] - cum[i + 1]);
  }

  ans[v] += accumulate(tmps.begin(), tmps.end(), int64_t(0));
  ans[v] *= 2;
  ans[v]++;
}
