...
signed main(int argc, char* argv[]) {
  ...
  int64_t ans = 0;
  rep(i, N) {
    int64_t max_l                = min(i, N - i - 1);
    function<bool(int64_t)> Comp = [&](int64_t l) {
      if (i - l - 1 < 0) {
        return true;
      }
      return a[N - l - 1] + a[i - l - 1] - 2 * a[i] < 0;
    };
    int64_t l   = LowerBound<int64_t>(0, max_l + 1, Comp);
    l           = min<int64_t>(max_l, l);
    int64_t tmp = cum[i] - cum[i - l] + cum[N] - cum[N - l] - (2 * l) * a[i];
    ChMax(ans, tmp);
  }
  cout << ans << endl;
