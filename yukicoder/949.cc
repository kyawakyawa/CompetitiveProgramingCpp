...
signed main(int argc, char* argv[]) {
  ...
  int64_t N;
  In(N);
  vector<int64_t> A(N + 1), B(N + 1), D(N);
  rep(i, N + 1) In(A[i]);
  rep(i, N + 1) In(B[i]);
  rep(i, N) In(D[i]);

  auto dp = MakeNdVector(N + 1, N + 1, int64_t(N));

  ALL(sort, D, greater<int64_t>());
  ...
  rep(i, N) {
    rep(j, i + 1) {
      ...
      auto func = [&](const int64_t l, const int64_t ab) {
        int64_t m = (dp[i][j] < 0 ? 1000000 : D[dp[i][j]]);
        ChMin(m, ab);
        int64_t k = lower_bound(D.begin() + (dp[i][j] + 1), D.end(), m,
                                greater<int64_t>()) -
                    D.begin();
        ChMin(dp[i + 1][l], k);
      };
      func(j, A[j] + B[i - j + 1]);
      if (j + 1 <= N) func(j + 1, A[j + 1] + B[i - j]);
    }
  }
  int64_t ans = 0;
  rep(i, N + 1) {
